...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light">
            ...
            <div class="row">
                <div class="col-2 side-bar">
                    ...
                    <ul>
                        ...
                        @foreach ($note as $tag_name => $notes)
                            <li><a href="/filtered/{{$tag_name}}">{{$tag_name}}</a></li>
                                <br>
                        @endforeach
                    </ul>
                </div>
                <div class="col-10">
                    <div class="row note-row">
                        @if(!session("results"))
                            @foreach ($note as $tag_name => $tagNotes)
                            <div class="col-md-4">
                                <div class="row">
                                    <h4 style="font-weight:500;background-color: white;width:170px;padding:12px;border:1px solid rgb(220, 220, 220);border-radius:10px;">{{ $tag_name }}</h4>
                                </div>
                                <br><br>
                                @foreach ($tagNotes as $noteItem)
                                    <div class="row note-container" style="background-color:white;padding:15px;width:250px;border-radius:10px;border:1px solid rgb(220, 220, 220)">
                                        <a href="/editnote/{{$noteItem->note_id}}" style="text-decoration:none;color:black">
                                            <h6 style="color: rgb(46, 46, 46);font-weight:700">{!! $noteItem['title'] !!}</h6>
                                            <p style="color: rgb(207, 207, 207)!important ;font-size:13px;width:200px;">{!! $noteItem['content'] !!}</p>
                                            <p style="color: rgb(197, 197, 197) !important;font-size:12px;width:200px;">Updated at {{$noteItem['updated_at']}}</p>
                                        </a>
                                        <a href="/deletenote/{{$noteItem->note_id}}" class="delete-button"><i class="bi bi-trash3-fill"></i></a>
                                    </div>
                                    <br><br>
                                @endforeach
                            </div>
          ...
          <button style="display: none" type="button" class="" id="btn" data-bs-toggle="modal" data-bs-target="#exampleModal">
              ...
              <div class="modal-content">
                ...
                <div class="modal-body">
                    @if(session("noteAdded"))
                        ...
                        {{session()->forget("noteAdded")}}
                    @endif
                    @if(session("noteUpdated"))
                        <p>Note Updated Successfully</p>
                        {{session()->forget("noteUpdated")}}
                    @endif
                    @if(session("noteDeleted"))
                        <p>Note Deleted Successfully</p>
                        {{session()->forget("noteDeleted")}}
                    @endif
                </div>
